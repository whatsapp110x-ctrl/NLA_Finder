...
#
# Timeout (in seconds) for network operations.  If ``DEFAULT_TIMEOUT`` is
# not set in the environment, we disable the timeout by default.  Setting
# the variable to a numeric string will override the default.  A value
# of ``None`` signals no timeout limit.
_timeout_env = os.getenv("DEFAULT_TIMEOUT")
if _timeout_env is None or _timeout_env == "" or _timeout_env.lower() == "none":
    # No timeout limit when unset or explicitly set to "none".
    DEFAULT_TIMEOUT: float | None = None
else:
    try:
        DEFAULT_TIMEOUT = float(_timeout_env)
    except ValueError:
        # Fallback to no timeout if the environment variable is not a valid number
        DEFAULT_TIMEOUT = None

# The maximum number of concurrent scanning connections.  This may be
# overridden via the MAX_CONCURRENT_SCANS environment variable.  A lower
# default (50) is used to avoid exhausting system resources by default.
